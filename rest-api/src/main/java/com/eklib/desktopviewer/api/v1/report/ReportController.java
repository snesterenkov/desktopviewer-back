...
public class ReportController {
    ...
    @ResponseBody
    public List<WorkDiaryDTO> getHoursByUserFromPeriod(@RequestParam(value = "startDate", required = true) @DateTimeFormat(pattern="yyyy-MM-dd") Date startDate,
                                      @RequestParam(value = "endDate", required = true) @DateTimeFormat(pattern="yyyy-MM-dd") Date endDate,
                                      @RequestParam(value = "client", required = false) String client){
        return reportService.getWorkingHoursByTimePeriod(startDate, endDate);
    }
